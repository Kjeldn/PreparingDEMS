...
def CannyLines(pixel_size,edgemap,gradientMap,orientationMap,maskMap,gradientPoints,gradientValues,edgemap_A):
    ...
    residualMask = (edgemap_A/255) - maskMap2
    for i in range(len(edgeChainsE)):
        ...
        for m in range(len(begin)):
            ...
            while b_x >= 0 and b_y >= 0:
                b_x,b_y = META.next2(b_x,b_y,rows,cols,residualMask,0,s)
                if b_x <= ceil_x:
                    break
                if b_x >= 0 and b_y >= 0:
        ...
        for n in range(len(end)):
            ...
            while e_x >= 0 and e_y >= 0:
                e_x,e_y = META.next2(e_x,e_y,rows,cols,residualMask,1,s)
                if e_x >= ceil_x:
                    break
                if e_x >= 0 and e_y >= 0:
