...
def pixelswitch(ps1,ps2,edgeChainsE,edgemap,gradientMap,orientationMap,maskMap,gradientPoints,gradientValues):
    ratio = ps1/ps2
    if ((ratio).is_integer()==False):
        print('Ratio between pixelsizes is not suitable for masking')
    else:
        ratio = int(ratio)
        MASK = np.zeros(edgemap.shape)
        for chain in edgeChainsE:
            for point in chain:
                MASK[(point[0]-1)*ratio:(point[0]+2)*ratio,(point[1]-1)*ratio:(point[1]+2)*ratio]=1
    MASK_INV = np.ones(MASK.shape)- MASK
    edgemap = edgemap - (MASK_INV*255)
    gradientMap = MASK * gradientMap
    orientationMap = MASK * orientationMap
    for i in range(len(gradientPoints)-1,-1,-1):
        if ((MASK[gradientPoints[i][0],gradientPoints[i][1]]==1)==False):
            del gradientPoints[i]
            del gradientValues[i]
