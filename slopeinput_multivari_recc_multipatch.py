...
for i in range(len(path)):
    ...
    exec("slope"+str(i)+" = calculate_slope(file"+str(i)+")")
    exec("mask = np.zeros(array"+str(i)+".shape)")
    exec("mask[slope"+str(i)+"<=0]=1")
    exec("mask"+str(i)+" = mask")
    exec("slope"+str(i)+"[slope"+str(i)+"<=0]=0")
    mask_f=ndimage.gaussian_filter(mask, sigma=20, order=0)    
    exec("img = slope"+str(i)+"*(255/np.max(slope"+str(i)+"))")
